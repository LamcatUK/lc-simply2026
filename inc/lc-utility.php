...
function contact_phone( $atts = array() ) {
	...
	if ( get_field( 'contact_phone', 'options' ) ) {
		...
		$icon_html   = ( 'true' === $atts['icon'] || true === $atts['icon'] ) ? '<i class="bi bi-telephone-fill"></i> ' : '';
...
function contact_email( $atts = array() ) {
	...
	if ( get_field( 'contact_email', 'options' ) ) {
		...
		$icon_html        = ( 'true' === $atts['icon'] || true === $atts['icon'] ) ? '<i class="bi bi-envelope-fill"></i> ' : '';
...
function lc_cf7_honeypot_form_tag_handler( $tag ) {
	...
	$html = sprintf(
		...
		esc_html( ! empty( $atts['message'] ) ? $atts['message'] : __( 'If you are human, leave this field blank.', 'lc-simply2026' ) ),
		$atts,
		$validation_error
	);
...
function lc_cf7_honeypot_validation_filter( $result, $tag ) {
	...
	if ( ! empty( $value ) ) {
		$result->invalidate( $tag, __( 'Spam detected.', 'lc-simply2026' ) );
	}
